function BotDetector(args) {
	...
	if (selectedTests.length == 0 || selectedTests.indexOf(BotDetector.Tests.WHEEL) != -1) {
		self.tests[BotDetector.Tests.WHEEL] = function() {
			var e = function(event) {
				console.log('event'+event)
				self.tests[BotDetector.Tests.WHEEL] = true;
				self.update()
				self.unbindEvent(window, BotDetector.Tests.WHEEL, e)
				self.unbindEvent(document, BotDetector.Tests.WHEEL, e)
			};
			self.bindEvent(window, BotDetector.Tests.WHEEL, e);
			self.bindEvent(document, BotDetector.Tests.WHEEL, e);
		};
	}
	...
	if (selectedTests.length == 0 || selectedTests.indexOf(BotDetector.Tests.DEVICE_ORIENTATION) != -1) {
		self.tests[BotDetector.Tests.DEVICE_ORIENTATION] = function() {
			var e = function(event) {
				...
				if((event.alpha || event.beta || event.gamma) && !isFirefox) {
					var alpha = event.alpha;
					var beta = event.beta;
					var gamma = event.gamma;
					if (!self.lastOrienData) {
						self.lastOrienData = {
							alpha: alpha,
							beta: beta,
							gamma: gamma
						};
						self.lastOrienDataCount = 0;
					}
					else {
						self.lastOrienDataCount ++;
						var movement = alpha != self.lastOrienData.alpha || beta != self.lastOrienData.beta || gamma != self.lastOrienData.gamma;
						self.lastOrienData = { alpha: alpha, beta: beta, gamma: gamma }
						if (movement && self.lastOrienDataCount > 3) {
					...
					self.tests[BotDetector.Tests.DEVICE_ORIENTATION] = false;
					self.unbindEvent(window, BotDetector.Tests.DEVICE_ORIENTATION, e);
				}
			}
		...
		self.tests[BotDetector.Tests.DEVICE_ORIENTATION_MOZ] = function() {
			var e = function(event) {
				if(event.x || event.y || event.z) {
					...
					var y = event.y;
					var z = event.z;
					if (!self.lastOrienMozData) {
						self.lastOrienMozData = {
							x: x,
							y: y,
							z: z
						};
						...
						var movement = x != self.lastOrienMozData.x || y != self.lastOrienMozData.y || z != self.lastOrienMozData.z;
						self.lastOrienMozData = { x: x, y: y, z: z }
						if (movement) {
							self.tests[BotDetector.Tests.DEVICE_ORIENTATION_MOZ] = true;
							self.update();
							self.unbindEvent(window, BotDetector.Tests.DEVICE_ORIENTATION_MOZ, e);		
						}
					}
				}
			}
